use glium::{backend::Facade, glutin::api::cgl::display, index::NoIndices, uniform, Program, Surface, Texture2d, VertexBuffer};
use rusttype::{point, Font, Scale};

use crate::geometry::{Vertex, SQUARE};
...
impl TextRenderer {
    ...
    pub fn new<T: Facade>(display: &T) -> Self {
        let program = Program::from_source(display, TEXT_RENDER_VERTEX_SHADER, TEXT_RENDER_FRAGMENT_SHADER, None).unwrap();
        ...
        Self { font: Font::try_from_bytes(include_bytes!("../fonts/OpenSans-bold.ttf")).unwrap(), program, vertex_buffer, index_buffer, prev_text: None, prev_texture: None }
    }
    ...
    pub fn render_text_to_texture<T: Facade>(&self, display: &T, text: &str) -> Texture2d {
        ...
        for glyph in glyphs {
            ...
            if let Some(bounding_box) = glyph.pixel_bounding_box() {
                glyph.draw(|x,y,v| {
                    result[(bounding_box.min.y as u32 + y) as usize][(bounding_box.min.x as u32 + x) as usize] = (255, 0, 255, (v*255.0) as u8);
                });
            }
        }
